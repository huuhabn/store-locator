...
- **Faster `/stores` REST endpoint.** Eliminated a per-store N+1 query problem in the store list: brand/country now read from the object-term cache (`get_the_terms()` instead of `wp_get_post_terms()`), featured-image attachments are bulk-primed in one batch instead of loaded individually per store, and the query explicitly primes the post-meta/term caches for the whole page. On a 500-store page this cuts roughly ~1,500 extra queries down to a handful.
- **Faster `/filters` REST endpoint.** The brand/country/city filter payload is now cached in a transient (`asl_filters_payload`) and only rebuilt when a store actually changes, so the endpoint stops re-running its term/meta queries on every locator page load. The cache is busted on store save, delete, and status change (publish/trash/untrash), plus any brand/country term create/edit/delete. Also removed a per-country N+1 by bulk-priming the custom flag-image attachments, and split the method into focused helpers.
